...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/conf/aria2.conf"
	...
	wget --no-check-certificate -N "https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/sh/update_bt-tracker.sh" && chmod +x update_bt-tracker.sh
	[[ ! -s "update_bt-tracker.sh" ]] && echo -e "${Error} Aria2 bt-tracker 更新文件下载失败 !" 
	echo '' > aria2.session
}
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/conf/aria2_service_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/conf/aria2_service_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Set_aria2_RPC_dir(){
	...
	stty erase '^H' && read -p "(默认位置: /home/down/file/down/):" aria2_RPC_dir
	[[ -z "${aria2_RPC_dir}" ]] && aria2_RPC_dir="/home/down/file/down/"
	echo
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/Tzhzjwd/docker/master/debian-caddy-aria2/sh/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -N --no-check-certificate https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/sh/aria2.sh && chmod +x aria2.sh
