...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/conf/caddy_service_centos -O /etc/init.d/caddy; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/conf/caddy_service_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
...
Conf_caddy(){
	cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/zhzjwd/docker/master/debian-caddy-aria2/conf/Caddyfile"
	[[ ! -s "Caddyfile" ]] && echo -e "${Error} Caddy 配置文件下载失败 !" && exit 1
}
install_caddy(){
